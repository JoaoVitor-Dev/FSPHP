...
if (!file_exists($file) || !is_file($file)){
    $fileOpen = fopen($file, "w");  //o 'w' significa que o arquivo será aberto para leitura e gravação
    fwrite($fileOpen, "Linha 01" . PHP_EOL);
    fwrite($fileOpen, "Linha 02" . PHP_EOL);
    fwrite($fileOpen, "Linha 03" . PHP_EOL);
    fwrite($fileOpen, "Lorem Ipsum is simply dummy text of the printing and typesetting industry");
    fclose($fileOpen);
}else{
    var_dump(
        file($file), //file faz a leitura do arquivo
        pathinfo($file)
    );
}
